...
## Description
Release - This phase focuses on achieving the project goals: Maintain current calendar hover animation behavior; investigate perceived reset after hover-out, Fix and cleanup hover/camera/shader/reset logic without visual regressions, Preserve performance and UX; keep config-driven behavior with hover-effects.json/presets.json. Technology stack: Three.js, GLSL shaders, JavaScript (ES modules), CSS, Webpack, JSON configs (hover-effects.json, presets.json).
